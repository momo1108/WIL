...
- Ubuntu Server의 경우
  ...
  VM-update vmware tools-mount /dev/cdrom /media/cdrom
  ...
  tar xvf VM(tab) /vmtool

  cd vmtool/vm(tab)

  ./vm(tab)

  이제 mount를 하면 공유폴더 경로가 보인다.
...
- 파일 소유권(Ownership)
  ...
  - ex) # chown ubuntu.ubuntu sample.txt
    ...
    \# chown ubuntu sample.txt ← 소유자만 변경

    \# chgrp ubuntu sample.txt ← 소유그룹만 변경
...
- 링크
  ...
  - 심볼릭 링크(symbolic link, soft link)
    - 새로운 inode2를 만들고, 데이터는 원본 파일의 위치를 가리킴 효과
...
- find [경로] [옵션] [조건] [action] : 기본 파일 찾기
  ...
  - [action] -print(디폴트), -exec(외부명령 실행)
    ...
    - \# find /home -name "*.swp" -exec rm {} \; ← ; 는 리눅스에서 명령어를 이어서 실행하라는 명령이므로 명령어의 종료를 위한 ; 라는것을 알리기 위해 escape(\\) 시켜준다.
